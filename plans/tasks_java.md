...
2. Решите любую задачу из первого раздела [задачника](https://ivtipm.github.io/Programming/Files/spisocall.htm) согласно своему варианту или предложите свою задачу сопоставимой сложности или немного сложнее (нужно согласовать с преподавателем).
3. Создайте *отчёт* о компилировании программ в консоли (PowerShell, bash) и запуске. 
...
4. Создайте отдельную функцию для вычислений. Вынесите её в отдельный модуль (java файл).
5. Сделайте автоматическое тестирование программы с помощью оператора `assert`. Для функций (методов), вычисляющих значения, сделайте минимум три проверки. Рассмотрите крайние случаи. Напишите поясняющие комментарии См. пример ниже.
...
Полностью выполните задание за 3 недели.
...
# 3. Конспект и отчёт по вашей любимой среде разработки для Java
   - Режимы запуска и компиляции: Сборка, запуск без отладки, запуск с отладкой.
   - Отладка. Точки останова. Режимы выполнения. Просмотр значений переменных. Стек вызовов.\
   Объясните на примере одной из ваших программ где есть циклы.
   - Файлы и папки проекта.\
   Какие файлы и папки стоит добавлять к отслеживанию гитом?
   - Часто используемые горячие клавиши
   - Средства рефакторинга
   - Задание параметров для компилятора (оптимизация, версия стандарта Java и т.п.) через общую таблицу параметров и через ручную запись аргументов компилятора.
   - Задание параметров для виртуальной машины
   - Задание параметров для программы
   - todo комментарии. Зачем нужны? Как создать? Как посмотреть список?

*Выполните задание за 1 неделю*

# 4. Конспекты по основам языка
...
1. Оператор (statement) `switch case`
1. Области памяти в JVM: стек, куча, Metaspace (статическая область). [Подробнее](../Java/studbook/20_memory.md)

*Выполните задание за 3 недели*



# 5. Работа с массивами, файлами, исключительными ситуациями
1. Сделайте задачу из 15 строки задачника:
    ...
    - функция для заполнения массива случайными числами из званного диапазона
    - напишите тесты
