...
1. Создайте *отчёт* о компилировании программ в консоли (PowerShell) и запуске. 
...
1. Сделайте автоматическое тестирование программы с помощью оператора `assert`. Для функций, вычисляющий значений сделайте минимум три проверки. Рассмотрите крайние случаи. Напишите поясняющие комментарии.
1. *Бонус: Добавьте в программу интерфейс командной строки (используйте параметр `argv`).*
...
3. Создайте в текстовом редакторе (например Sublime Text) файл с расширением .java и исходным кодом простой программы на Java (например Main.java):
```java
...
# запуск из текущей папки (запустится файл Main.cla)
java Main
```
...
1. Выберете рекомендации по стилю кодирования (styles guidelines) на C++, которые нравятся вам. Приведите в конспекте наиболее важные и часто используемые рекомендации.
1. Значение по умолчанию для параметров функций.
...
# 2. Конспект и отчёт по вашей любимой среде разработки для С++
