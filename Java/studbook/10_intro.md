...
В Java `final` практически заменяет ключевое слово `const` из других языков программирования. Но есть и отличие -- final переменную можно инициализировать не сразу.
...
// задание значения
fi = 1.618;    // из пропорции золотого сечения

fi = 2; // Ошибка! Изменять значение запрещено!
```

### Ключевое слово `var` (Java 10+)

Начиная с Java 10 можно заменить явный тип на `var`, если компилятор выводит его из выражения справа:
...
`var` **не** делает Java динамической — тип определяется на этапе компиляции и не меняется. Уместен, когда тип очевиден из инициализатора. Не злоупотребляйте, если это ухудшает читаемость.
...
## 6. Пакеты и стандартная библиотека Java

Материал вынесен в отдельный файл: [16_packages.md](16_packages.md).

Кратко:
- `import` -- подключение классов из пакетов
- `import static` -- импорт статических методов/констант
- `java.lang` подключается автоматически
- Шпаргалка по самым полезным классам (`String`, `Math`, `Scanner`, `ArrayList` и др.)
