...
- Оператор `%` дает остаток делимого.
...
- **Явное преобразование** используйте, когда нужен контроль над типом результата или когда переводите `long` → `int`.
...
## 12. Объектные обёртки, автоупаковка/автораспаковка

Для каждого примитивного типа есть класс-обёртка: `Integer` (для `int`), `Double`, `Boolean`, `Character` и т.д. Обёртки нужны, когда требуется объект (например, в коллекциях `ArrayList<Integer>`).

**Автоупаковка** — автоматическое превращение примитива в обёртку, **автораспаковка** — обратно:
...
### Сравнение `==` vs `.equals()` с классами-обёртками

`==` для обёрток сравнивает **ссылки**, а не значения. Кэширование мелких чисел (-128…127) может запутать:

```java
Integer x = 127;
Integer y = 127;
System.out.println(x == y);      // true (оба из кэша)

Integer p = 200;
Integer q = 200;
System.out.println(p == q);      // false (!) — разные объекты
System.out.println(p.equals(q)); // true — сравниваются значения
```

**Правило:** для сравнения **любых объектов** (включая обёртки, строки, свои классы) используйте `.equals()`. `==` оставьте для примитивов и проверки, ссылаются ли две переменные на один и тот же объект.

## 13. Преобразование строки в число и обратно
...
long l = Long.parseLong("123");        // → 123L
```

При неверном формате выбрасывается `NumberFormatException`:
...
String s4 = "" + 42;                     // "42" (конкатенация)
```

`String.valueOf(...)` — универсальный метод, работает с любым типом.

Для вывода чисел стоит использовать форматирование строк (например printf) вместо функции toString. 

А для создания строк с форматированием используйте функцию format из типа String:

```java
String.format("%.2d", 123);
```

TODO: пересмотреть структуру файла, в частности часть про хранение вещественных чисел.
