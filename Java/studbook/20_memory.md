...
- **Python** использует **счётчик ссылок** (reference counting). Каждый объект хранит число ссылок на себя; когда счётчик падает до нуля, объект удаляется немедленно. Циклические ссылки обрабатываются отдельным GC.
...
- Размер стека вызова можно задать при запуске JVM. Например, `-Xss2m` — увеличивает размер стека до 2 МБ. В IntelliJ IDEA можно задать размер стека в настройках запуска программы в поле VM options.
...
`new` — это аналог `malloc` в C, но управляемый JVM. Он возвращает *адрес* выделенной памяти. В отличие от C, `new` никогда не возвращает `NULL` — если памяти в куче недостаточно, JVM выбрасывает `OutOfMemoryError`, и программа завершается аварийно.
...
Здесь `Scanner sc` — объявление переменной (тип + имя), а `new Scanner(System.in)` — выделение памяти и инициализация объекта. Это два разных действия: объявление говорит «будет переменная-ссылка типа `Scanner`», а `new` создаёт сам объект в куче. Без `new` в `sc` нельзя будет ничего сохранить — компилятор не даст использовать неинициализированную переменную.
...
int[] numbers;             // объявлена ссылочная переменная, но память НЕ выделена
// numbers[0] = 5;        // ОШИБКА (!): NullPointerException — numbers равна null
numbers = new int[5];      // теперь память выделена, можно работать
```
