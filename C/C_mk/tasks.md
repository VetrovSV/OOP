...
Если не указано иного, считайте, что исходные данные - вещественные числа.
...
* В комментарии опишите что функция делает, что возвращает. Если параметры имеют особый смысл, не являются числами переменными в абстрактной формуле, а например содержат размер массива или т.п. описывайте их смысл тоже. 
* В вашей программе укажите, какие именно параметры являются фактическими а какие формальными.


Помните, что размер массива - часть его типа данных. 
Пример передачи статического массива в функцию
```c
// Количество элементов в массиве
#define N 128

// функция принимает массив arr, и возвращает сумму первых n его элементов
void sum(int arr[N], unsigned n) {
    int sum = 0;
    for (int i = 0; i<n; i++){
        sum = sum + arr[i];
    }
...
int main(){
  ...
  int s1 = sum(a, n);       // так можно

  int b[20];
  int s2 = sum(b, n);       // так нельзя: функция принимает массив из 128 элементов, а передаётся массив из 20!
...
Создайте собственный тип данных, который описывает один контакт (пин) платы микроконтроллера. Пин характеризуется: номером (целое число 0-39), использовалась ли подтяжка (да\нет), напряжением (произвольное значение от -5 до 5).

Создайте массив из 10 или более элементов описывающих пины. Заполняйте все значения случайными числами.

Выведите массив на экран.

Запросите у пользователя несколько номеров пинов. 

Найдите среди введённых номеров пинов тот, который
- показывает максимальное напряжение (чётный вариант);
- показывает минимальное напряжение (нечётный вариант).
