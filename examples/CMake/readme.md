...
Подготовка сборки, временные файлы сборки и исполняемый файл будут сохранены в папку my_build (`-B ../my_build`), находящуюся на одном уровне с папкой src, `.` - имя текущего кеталога.
```bash
cmake -B ../my_build .
```
