...
Подготовка сборки, временные файлы сборки и исполняемый файл будут сохранены в папку my_build, находящуюся на одном уровне с папкой src, `.` - имя текущего аталога.
```bash
cmake -B ../my_build .
```
