...
JDK - 21 ~ 65 class file format major version\
JDK - 22 ~ 66 class file format major version\
JDK - 23 ~ 67 class file format major version

Среда исполнения Java (JRE) обычно поддерживает целый диапазон версий class файлов.
Например, последняя на март 2025 года JRE поддерживает версии 45 .. 67.


### jar файлы

Перед созданием Jar файлов ориентируйтесь на версии JRE, под которыми будет запускаться программа. Нередко, это не самые последние версии Java. Поэтому задавай
...
#### Запуск
Виртуальная машина Java должна быть установлена и доступна в Path. Версия виртуальной машины должна быть выше или такой же как JDK, с помощью которого получен файл.
...
**Запуск в Ubuntu**\
Запустить jar файл можно непосредственно, как и любой другой исполняемый файл - по двойному клику.

Если запуск недоступен, то можно выбрать вариант запуска из контекстного меню:
<img src="assets/Ubuntu_run_jar.png" width=300>

Для запуска может понадобится задать атрибут файла: запускаемый
```bash
chmod +x my_prog.jar
```
...
4. В окне настроек JAR файла указать имя главного класса приложения (Main Class). Будет автоматически создан файл MANIFEST.MF - файл описания Jar архива. 
5. [ОК]
...
В intelij IDEA создание JAR файлов на основе специального шаблона JavaFX приложений не работает с параметрами по умолчанию.

Решение: Создать новый главный класс, в котором явно вызывать статический метод `launch` из стандартного JavaFX класса Application.
```java
...
- **Многострочный комментарий** (стандартный формат):
  ```java
  /**
   * Описание элемента.
   */
...
Все комментарии записываются строкой выше того, что они  комментируют.

Нет способа сделать Javadoc комментарий в той же строке, что и коммутируемый код.


Наиболее часто используемые теги
* `@param` - Описание параметра метода. Пример: `@param name Имя пользователя`
* `@return`- Описание возвращаемого значения. Пример: `@return Возраст пользователя`
* `@throws` - Описание исключения, которое может быть выброшено. Пример: `@throws IOException Ошибка ввода-вывода`
* `@see` - Ссылка на другой класс/метод. Пример: `@see String#length()`
* `@deprecated` - Пометка устаревшего элемента. Пример: `@deprecated Используйте метод X`
...
Однострочный комментарий к методу, без тегов
```java
/// Возвращает команду склеивания файлов, нужна для показа пользователю
public String merge_cmd(){
```

Многострочный комментарий с тегами
```java
/**
